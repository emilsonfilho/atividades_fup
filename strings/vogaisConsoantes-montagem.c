...
int main()
{
    String name = readString(), vcName = createString();

    for (int i = 0; i < name.length; i++)
    {
        char ch = name.value[i];
        if (ch != ' ') {
            char encryptedChar = isVowel(tolower(ch)) ? 'v' : 'c';
            vcName.value = concatChar(vcName.value, &vcName.length, &vcName.memory, encryptedChar);
        } else {
            vcName.value = concatChar(vcName.value, &vcName.length, &vcName.memory, ch);
        }
    }
